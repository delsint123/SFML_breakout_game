 ...
 * Purpose: Final addition to make breakout game
 * -------------------------------------------------------- */
...
void setup(Ball &ball, Borders &borders, Brick bricks[BRICK_ROWS][BRICK_COLUMNS],
           MovingBlock &paddle);
bool update(Direction &input, Ball &ball, float delta, Borders theBorders,
            Brick bricks[BRICK_ROWS][BRICK_COLUMNS], MovingBlock &paddle, bool &started, int &hit, int &points);
void render(sf::RenderWindow &window, Ball ball, float delta, Borders theBorder,
            MovingBlock paddle, Brick bricks[BRICK_ROWS][BRICK_COLUMNS]);
int getCollisionPoint(Ball *ptrBall, Block *ptrBlock);
bool collisionCheck(Ball *ptrBall, Block *ptrBlock);
bool blockCollisionCheck(Block *ptrMovingBlock, Block *ptrStationaryBlock);
bool doCollisionChecks(Ball &ball, MovingBlock &paddle,
                       Brick bricks[BRICK_ROWS][BRICK_COLUMNS],Borders theBorders, int &hit);
...
int main() {
    ...
    setup(theBall, theBlocks, theBricks, thePaddle);
    ...
    while (!gameOver)
    {
        ...
        if (delta >= FRAME_RATE) {    // if we have made it at least a full frame time
            ...
            gameOver = update(userInput, theBall, delta, theBlocks,
                              theBricks,thePaddle, started, hits, score);
        ...
        // Render the window
        render(window, theBall, delta, theBlocks, thePaddle, theBricks);
...
void setup(Ball &ball, Borders &borders, Brick bricks[BRICK_ROWS][BRICK_COLUMNS],
    ...
    for(int row = 0; row < BRICK_ROWS; row++) {
        ...
        for(int column = 0; column < BRICK_COLUMNS; column++) {
            ...
            pNext->block.blockShape.setSize(sf::Vector2f(pNext->block.width, pNext->block.height));
            pNext->block.blockShape.setFillColor(pNext->block.blockColor);
            pNext->block.blockShape.setPosition(pNext->block.left, pNext->block.top);
...
/**
 * Checks collision between a moving and stationary block
 * @param pBlock1 - moving block
 * @param pBlock2 - stationary block
 * @return bool if there is a collision
 */
bool blockCollisionCheck(Block *ptrMovingBlock, Block *ptrStationaryBlock){
    bool collides = false;

    if (ptrMovingBlock->left < ptrStationaryBlock->left + ptrStationaryBlock->width &&
        ptrMovingBlock->left + ptrMovingBlock->width > ptrStationaryBlock->left &&
        ptrMovingBlock->top < ptrStationaryBlock->top + ptrStationaryBlock->height &&
        ptrMovingBlock->top + ptrMovingBlock->height > ptrStationaryBlock->top) {
        collides = true;
    }
...
/**
 * Holds all collision checks for the game
 * @param ball - ball properties
 * @param paddle - paddle properties
 * @param walls - wall properties
 * @param bricks - brick properties
 * @param hit- speed up the ball for number of hits
 * @return - bool for game over
 */
bool doCollisionChecks(Ball &ball, MovingBlock &paddle,Brick bricks[BRICK_ROWS][BRICK_COLUMNS],
                       Borders theBorders, int &hit, int &points) {
    bool gameOver = false;
    points = 0;
    hit = 0;
    ...
    //collision check for paddle and left and right walls
    //left wall
    if(blockCollisionCheck(&paddle.theMovingBlock, &theBorders.leftBlock)) {
        paddle.velocityX = 0.0;
        paddle.theMovingBlock.left = theBorders.leftBlock.left + theBorders.leftBlock.width + 1;
    }

    //right wall
    else if(blockCollisionCheck(&paddle.theMovingBlock, &theBorders.rightBlock)) {
        paddle.velocityX = 0.0;
        paddle.theMovingBlock.left = theBorders.rightBlock.left - paddle.theMovingBlock.width - 1;
    }
...
bool update(Direction &input, Ball &ball, float delta, Borders theBorders,
    ...
    //increases velocity as hits increases
    if(hit == 4) {
        ball.velocityX += 10.5;
        ball.velocityY += 10.5;
    }
    ...
    gameOver = doCollisionChecks(ball, paddle, bricks, theBorders, hit, points);
 ...
 * @param bricks - structure array holds properties of bricks
 */
